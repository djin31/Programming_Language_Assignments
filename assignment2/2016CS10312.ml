...
let rec evalnumber e = match e with
					...
					|Proj(e1,Tup(l))-> evalnumber(givepos l (evalnumber e1) 0)
...
let rec evalbool e = match e with
					...
					|Proj(e1,Tup(l))-> evalbool(givepos l (evalnumber e1) 0)
					|_-> raise NotCorrectType;;
...
(*-------- Examples -----------*)
(* P.S. eval function doesn't accept A-assignment as argument since the specifications given expect eval to be exp->answer *)
(* So eval function directly uses the table function, to modify the A-Assignment, change the definition of table *)
(* However the execute machine take A-Assignment as an argument, so any custom A-Assignment (Var(string) -> Answer) can be used for computing *)
...
let tup = Tup([x;y;z]);;
let proj = Proj(Const 2,tup);;														(* Projects y *)
...
let x1 = eval x;;															(* x1 should be 12 *)
let y1 = eval y;;															(* y1 should be 13 *)
let z1 = eval z;; 															(* z1 should be 25 *)
let tupcode = eval tup;;

x1 = execute ([],table,xcode);;												(* should evaluate to true if execute is correct *)
y1 = execute ([],table,ycode);;
z1 = execute ([],table,zcode);;
z1 = execute ([],table,projcode);;
...
eval x;;
execute ([],table,compile (Var("y")));;
eval x = execute ([],table,compile x);;

(* Example using a boolean variable in a tuple with mixed type *)
let index = Sub(Div(Add(Var("x"),Mul(Var("y"),Const(3))),Const(60)),Const(1));;		(* index = int(x/60) -1 = int((var(x) + 3*var(y))/60 - 1) = int((31 + 20*3)/60 -1) = 0 *)
...
(*Changed line 125 abs e to abs e1*)
(*Added 106 and 114*)