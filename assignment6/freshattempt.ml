...
let rec find_possibilities prog query e = match prog with 
      |[] -> e
      |(Fact(a1)::xs) -> 
            try 
                  let sol = mgu_atomic a1 query in
                  (find_possibilities xs query (e@[Fact(a1)]))
            with
            |NOT_UNIFIABLE -> (find_possibilities xs query e)
      |(Rule(a1,body)::xs) ->
            try 
                  let sol = mgu_atomic a1 query in
                  (find_possibilities xs query (e@[Rule(a1,body)]))
            with
            |NOT_UNIFIABLE -> (find_possibilities xs query e);;


let rec solve_query prog query qstack pstack unif= match qstack with
      |[] ->      flush Pervasives.stdout;
                  Printf.printf "\n Press ; to continue\n";
                  print_unifs (filter_unifier goalvars unif);
                  flush Pervasives.stdout
      |(x::xs) -> 
